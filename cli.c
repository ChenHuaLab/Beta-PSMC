...
#define PSMC_VERSION "1.1" //change 0.6.5-r67 to 1.1 by liujf 2019-11-11
...
{
	fprintf(stderr, "\n");
	fprintf(stderr, "Program: beta-psmc (Pairwise SMC Model + Beta distribution)\n"); //change psmc to beta-psmc and add Beta distribution by liujf 2019-11-11
	fprintf(stderr, "Version: %s\n", PSMC_VERSION);
	fprintf(stderr, "Contact: <https://github.com/chenh-big/Beta-PSMC/>\n\n"); //change address by liujf 2019-11-11
	fprintf(stderr, "Usage:   beta-psmc [options] input.txt\n\n"); //change psmc to beta-psmc by liujf 2019-11-11
...
//	fprintf(stderr, "         -I FLOAT    amplitude of random initialization for EM iteration [%lg]\n", p->ran_init);
	...
	fprintf(stderr, "         -B INT      discrete number of Beta distribution model; 1 to disable [5]\n"); //add by liujf 2019-07-10
	fprintf(stderr, "\n");
	psmc_delete_par(p);
	exit(1);
}
...
psmc_par_t *psmc_new_par()
{
	...
	par->alpha = 0.1;
	par->dt0 = -1.0;
	par->n_discrete = 5; //add by liujf 2019-07-10
	return par;
}
...
psmc_par_t *psmc_parse_cli(int argc, char *argv[])
{
	...
	psmc_par_t *par;
	par = psmc_new_par();
	while ((c = getopt(argc, argv, "i:t:l:r:N:p:o:dI:c:bSDT:B:")) >= 0) { //add B: by liujf 2019-07-18
		switch (c) {
		...
		case 'B': par->n_discrete = atoi(optarg); break; //add by liujf 2019-07-10
		default:  usage(par);
		}
	}
	...
	if (par->pre_fn) { // load parameters from input
		...
		par->n = par->n * par->n_discrete; //add by liujf 2019-07-10
	}
