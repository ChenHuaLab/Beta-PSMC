...
#define max2(a,b) ((a)>(b)?(a):(b)) // add by liujf 2019-07-17
...
typedef struct
{
	...
	int n_discrete; // discrete number of Beta distribution model add by liujf 2019-07-10
...
extern "C" {
	...
	// compute Beta distribution by liujf 2019-07-17
  ...
  FLOAT psmc_beta(FLOAT p, FLOAT q, int j, int k);
	...
	void psmc_print_data_final(const psmc_par_t *pp, const psmc_data_t *pd); // add by liujf 2019-07-24
	// simulate sequences
